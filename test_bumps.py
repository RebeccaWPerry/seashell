...
# first simple test image is a square of 0's on a background of 1's
square = ones([200,200])
square = square>0
square[50:120,50:130] = False

# second simple test image is a circle of 1's on a bg of 0's
rows, cols = mgrid[-100:100,-100:100]
circle = (rows**2+cols**2) < 650

def test_crop_size_on_square():
    res  = crop(square)
    assert sum(res[0])==0
...
def test_crop_size_on_square_with_padding():
    res  = crop(square,4)
    assert sum(res[0])==1264

def test_crop_location_on_square():
    res  = crop(square)
